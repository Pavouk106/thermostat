...
def window_main(window_type, mouse_click):
	...
	if window_type == 1:
		...
		button_on_off(0, u"Ohřev vody", u"(" + str(temp_values[4]) + "/" + str(temp_values[5]) + " " + u"\u00b0" + "C)", white, main_temp_vertical_divider + border, 0 + border, 130, 70, red_dark, red, green_dark, green)
		...
		if heating_time_end != heating_time:
			...
			if seconds < 10:
				...
				seconds = str(seconds)
			heating_time_show = "(" + minutes + ":" + seconds + ")"
		button_on_off(1, u"Ruční topení", heating_time_show, white, main_temp_vertical_divider + border, 0 + border + 70 + border, 130, 70, red_dark, red, green_dark, green)
		...
		display.blit(TextSurf, TextRect)
		TextSurf, TextRect = text_objects(str(wind_values[3]), text_button, white)
